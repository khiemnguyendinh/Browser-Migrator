...
class ChromiumWriter(BaseProfileWriter):
    ...
    def write_cookies(self, cookies: list) -> bool:
        ...
        data = [
            (
                c.host_key, c.name, c.encrypted_value, c.path, c.expires_utc, 
                int(c.is_secure), int(c.is_httponly), int(c.has_expires), int(c.is_persistent), c.samesite
            ) for c in cookies
        ]
    ...
    def write_passwords(self, passwords: list) -> bool:
        ...
        data = [
            (
                p.origin_url, p.action_url, p.username_element, p.username_value, 
                p.password_element, p.encrypted_password, p.date_created, p.times_used
            ) for p in passwords
        ]
    ...
    def write_profile(self, snapshot: ProfileSnapshot) -> bool:
        cpm_logger.info(f"Writing profile to {self.target_profile_path} using {self.adapter.browser_id} adapter")
