...
class Api:
    ...
    def get_browsers(self):
        ...
        try:
            ...
            self.browsers = [b for b in detected if b.is_installed and any(p.is_default for p in b.profiles)]
    ...
    def start_migration(self, source_id: str, target_id: str, options: dict = None):
        ...
        try:
            ...
            if success:
                return {"success": True, "message": "Successfully migrated data! Please restart the target browser."}
            else:
                return {"success": False, "message": "Migration failed. Please check the logs in ~/.cpm/logs."}
        except Exception as e:
